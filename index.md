...
the number of analysable mismatch trials for every session containing a
sensorimotor block.
...
:::{figure} ./images/figures/generated/supplementary-sensorimotor-running.svg
:label: fig-supp-sensorimotor-running
:enumerated: false
:alt: Three aligned panels listing the sixteen sensorimotor sessions ordered by block mean forward speed, the number of mismatch trials in which the animal was running at each of four thresholds, and the number of surviving trials in the worst event type relative to an eight-trial minimum.
:width: 100%

**Supplementary Figure 8.** Locomotion during the sensorimotor mismatch block across all 16 Neuropixels sessions containing that block. The block is a closed-loop visuomotor paradigm in which optic flow is generated by the animal's own locomotion, and a mismatch event transiently decouples the two. A stationary animal generates no flow, so there is nothing to decouple and the event is not a stimulus; gating mismatch trials on running is therefore a validity requirement rather than statistical hygiene. Forward speed comes from the NWB processed running series at 60 Hz in cm/s, with negative velocities set to zero. A trial qualifies as running only when mean forward speed reaches the threshold in **both** the 343 ms pre-event baseline window and the mismatch window, because a closed-loop mismatch requires flow to have been present before it was decoupled; requiring only the mismatch window would admit trials in which the animal began moving in response to the event. Each window contains at least 20 native samples. Block statistics span the full sensorimotor block from the stimulus table, and block running fractions use a strict comparison so they remain comparable with the running summaries reported elsewhere in this release. All three panels share one row axis of sessions ordered by block mean forward speed. **A,** Block mean forward speed, with the 5 cm/s gate marked. **B,** Qualifying trials of the 140 mismatch events at each reported threshold, darker cells indicating more qualifying trials. **C,** Qualifying trials for each of the four mismatch event types, of 35 presentations each, with cells coloured by whether that type reaches a pre-registered minimum of 8 trials. Availability is per event type rather than per session: mouse 832691 falls below the minimum only for motor halt, and mouse 830849 falls below it for motor omission and the 45° change while clearing motor halt and the 90° change. Locomotion is strongly bimodal: the median session speed is 1.00 cm/s, 10 of 16 sessions have a median speed of exactly 0.00 cm/s, and only 2 sessions clear the minimum in all four event types at 5 cm/s. Mouse 848387 retains 137 of 140 trials with 33 in its worst event type, and mouse 830794 retains 124 with 29; relaxing the gate to 1 or 2 cm/s admits three further sessions but leaves them at 6, 6, and 1 trials in their worst type at 5 cm/s. Mouse 830846, used in the previously released version of Figure 10, averages 1.12 cm/s with a median of 0.00 and retains 2, 4, 4, and 4 trials across the four event types at 5 cm/s, which is why Figure 10 uses mouse 830794. Independently of locomotion, the matched open-loop control block contributes only 8 trials per mismatch type, 32 in total, so the sensorimotor mismatch-versus-control comparison is trial-limited on the control side in every session. Values were measured from the public draft of [Dandiset 001637](https://dandiarchive.org/dandiset/001637/draft/files).
:::
