...
- `scripts/extract_neuropixels_event_responses.py` streams four versioned Neuropixels NWBs from one mouse, computes native 1 ms context and matched-control SDFs over −0.75 to 0.75 s (standard, sensorimotor, sequence) or −1.5 to 1.5 s (duration) with a 5 ms causal exponential time constant and 10τ support, retains every 1 ms mean-SDF sample, reads Units-table firing rates, sorter labels, and peak-to-valley durations, derives same-session optotagged SST and waveform-based RS/FS classes, maps exact areas through the Allen ontology, computes event-specific Rastermap 1.0 ranks, and writes committed metadata plus compressed quantized SDF atlases for the interactive unit-response explorer.
