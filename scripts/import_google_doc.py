...
Probe A in representative session `ecephys_830846_2026-03-09_10-32-54`. The
source view displays 100 ms of unaveraged AP voltage as a time-by-probe-length
image from 96 regularly spaced contacts and overlays 310 sorted-spike detections
from 167 of the 569 Probe A units. Each marker is placed at its spike time and
nearest displayed peak channel; selecting a marker or unit highlights its depth
and NWB waveform-spread band. QC-passing detections can be isolated with the
viewer toggle. The selected unit is paired with a 12 s elapsed-time trace of
20 ms binned spike rate and its actual peak-channel mean template waveform. No
stimulus annotations are shown. In the **Static** view, **A** shows the raw AP
image and detected spikes and **B** shows the selected unit's activity and
waveform. Unit templates, spike times, quality metrics, and electrode geometry
come from the public draft of
[DANDI:001637](https://dandiarchive.org/dandiset/001637/draft/files); the raw AP
image comes from the matched public compressed stream.
:::
...
All 399 NWB segmentation masks are outlined over a grayscale average projection.
...
are outlined over a grayscale channel-0 mean image. The NWB image axes are
transposed into display coordinates before applying the `PlaneSegmentation`
masks, following the source-processing convention. The background-intensity
...
`PlaneSegmentation` pixel masks, timestamps, and fluorescence traces are
streamed from the matched NWB in the public draft of
[DANDI:001424](https://dandiarchive.org/dandiset/001424/draft/files).
:::"""
