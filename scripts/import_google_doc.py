...
recording modalities. The **Neuropixels** tab displays 100 ms of unaveraged
Probe A AP voltage from representative session
`ecephys_830846_2026-03-09_10-32-54`, with 310 sorted-spike detections from 167
of 569 units overlaid at their spike times and nearest displayed peak channels.
...
**Mesoscope** tab outlines all 399 VISp 0 segmentation masks over a grayscale
average projection from `multiplane-ophys_832700_2026-01-29_11-18-09`; selection
reveals classification probabilities, footprint geometry, and a 30 s ΔF/F
trace. The **SLAP2** tab outlines all 45 DMD1 source masks over a grayscale mean
image from `SLAP2_796630_2025-08-28-14-25-34`; image axes are transposed into
display coordinates before applying the masks, and selection reveals a 30 s,
approximately 200 Hz ΔF/F trace. Its source-localization activity image is
available as an optional overlay. Microscopy background controls alter only
grayscale image intensity, while filter colors remain fixed. No tab shows
stimulus annotations. The stacked static fallback preserves the complete source
and selected-trace view for all three modalities. Data come from the public
